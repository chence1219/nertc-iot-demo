...
AudioService::~AudioService() {
    ...
    if (opus_decoder2_ != nullptr) {
        esp_opus_dec_close(opus_decoder2_);
    }   
#endif
...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    if (opus_decoder2_ == nullptr) {
        esp_opus_dec_cfg_t wake_opus_dec_cfg = OPUS_DEC_CFG(16000, 20);
        auto ret2 = esp_opus_dec_open(&wake_opus_dec_cfg, sizeof(esp_opus_dec_cfg_t), &opus_decoder2_);
        if (opus_decoder2_ == nullptr) {
            ESP_LOGE(TAG, "Failed to create wake word audio decoder, error code: %d", ret2);
        } else {
            ESP_LOGI(TAG, "Wake word opus decoder created successfully");
        }
    }   
#endif
#else
...
#else
            std::vector<int16_t> data;
            int samples = wake_word_->GetFeedSize(); // 160; // 10ms
            if (samples <= 0) {
                samples = 160; // 10ms;
            }
            if (ReadAudioData(data, 16000, samples)) {
...
void AudioService::ResetDecoder() {
    ...
    if (opus_decoder_ != nullptr) {
        ...
        esp_opus_dec_cfg_t opus_dec_cfg = OPUS_DEC_CFG(decoder_sample_rate_, decoder_duration_ms_);
        auto ret = esp_opus_dec_open(&opus_dec_cfg, sizeof(esp_opus_dec_cfg_t), &opus_decoder_);
        if (opus_decoder_ == nullptr) {
            ESP_LOGE(TAG, "Failed to create audio decoder, error code: %d", ret);
        }
    ...
    while (true) {
        ...
        if (!wake_word_opus_queue_.empty()) {
            ...
            if (opus_decoder2_) {
                ...
                pcm.resize(320); // 20ms 16000Hz mono
                esp_audio_dec_in_raw_t raw = {
                    .buffer = data.data(),
                ...
                esp_audio_dec_info_t dec_info = {};
                auto ret = esp_opus_dec_decode(opus_decoder2_, &raw, &out_frame, &dec_info);
                if (ret == ESP_AUDIO_ERR_OK) {
                    pcm.resize(out_frame.decoded_size / sizeof(int16_t));
                    std::memcpy(pcm.data(), out_frame.buffer, out_frame.decoded_size);
                    std::unique_lock<std::mutex> lock(wake_wake_pcm_buffer_mutex_);
                    wake_pcm_buffer_.insert(wake_pcm_buffer_.end(), pcm.begin(), pcm.end());
                    lock.unlock();
                } else {
                    ESP_LOGE(TAG, "Failed to decode wake word audio, error code: %d", ret);
                }
            }
        }
    }
